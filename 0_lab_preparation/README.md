...
{: .warning}
AI Document Intelligence リソースを作成する場合は、**East-US、West-US2、West-Europe** のいずれかのリージョンでリソースを作成してください。 そうしないと、リソースにアクセスしようとしたときに 404 エラーが発生する可能性があります。 ([Source](https://learn.microsoft.com/en-us/answers/questions/1514842/document-intelligence-ai-returns-404))

### 実践的な要件
- [Azure OpenAI] サービスにアクセスできることを確認します。
- [Azure ML] ワークスペースを設定し`<WORKSPACE_NAME>`、`、<RESOURCE_GROUP>`、`<SUBSCRIPTION_ID>` を取得します。 
- [Azure AI スタジオ]でプロジェクトを作成します。
- LLM トレーニングには、単一の NVIDIA A100 GPU ノード (**[Standard_NC24ads_A100_v4]**) をお勧めします。 低優先度の VM は、予算が限られている場合、または専用のクォータがない場合は選択します。 
- LLM サービスには、単一の NVIDIA V100 GPU ノード (**[Standard_NC6s_v3]**) をお勧めします。 

### Azure Developer CLI(azd)を使用したAzureリソースのデプロイ
最も簡単な方法をお探しの場合、このラボでは、Bicep を使用してすべてを簡単にプロビジョニングできます。 次の手順では、必要な Azure リソースをプロビジョニングします。 まだインストールしていない場合 CLI をダウンロードします。 [What is the Azure Developer CLI?](https://learn.microsoft.com/en-us/azure/developer/azure-developer-cli/overview?tabs=windows#a-sample-azd-workflow) 
...
azd up
```
**重要**: このコマンドによって作成されたリソースには、主に AI Search リソースからすぐにコストが発生することに注意してください。 これらのリソースは、コマンドが完全に実行される前にコマンドを中断した場合でも、コストが発生する可能性があります。 `azd down` を実行するか、リソースを手動で削除して、不要な支出を避けることができます。
...
### 注意事項
- **[Standard_NC6s_v3] GPU クォータの引き上げ (\*12 コア) を要求します。 
- プライベート環境で構成する場合は、サービスにアクセスするためのプライベートネットワークまたはVPNを設定します。
- **[低優先度 VM]** 可用性はリージョンによって異なる場合があります。
- Azure ML ワークスペース内のデータとモデルの格納に使用される任意の BLOB ストレージの接続を設定します。
- VM または GPU の必要に応じて、クォータの引き上げをリクエストします。
- Azure ML ワークスペースのネットワーク構成は、セットアップ後に変更できません。 必要に応じて、新しいワークスペースを作成します。
...
{: .note }
マネージド Onlie エンドポイントの場合、[Azure ML はデプロイのクォータの 20% を予約します]。 [^1]デプロイ内の VM SKU に対して特定の数のインスタンスを要求する場合、エラーの発生を回避するために、"ceil(1.2 × number of instances requested for deployment) × number of cores for the VM SKU" のクォータを使用できる必要があります。 たとえば、デプロイで `Standard_NC6s_v3` VM (6 コアが付属) の 1 インスタンスを要求する場合、12 コア (ceil(1.2 × 1 インスタンス) = 2、2 × 6 コア) のクォータが使用可能である必要があります。 

ワークショップ中に一般的な問題を回避するために、これらのポイントが守られていることを確認してください。

## 2.コンピュートインスタンスと設定ファイルのセットアップ
- 1️⃣ コンピューティングリソースを準備する
- 2️⃣ リポジトリをクローンする
- 3️⃣ `.env` ファイルを作成し、Azure OpenAI と Azure Document Intelligence の詳細を追加します
- 4️(2) セットアップconfig.yml
-  セットアップの検証を開始する
...
2. コード開発には、**[Standard_E2as_v4](AMD 2コア、16GBのRAM、32GBのストレージ)または**[Standard_DS11_v2]**(Intel 2コア、14GBのRAM、28GBのストレージ、GPUなし)の`Review+Create`ボタンをクリックしてコンピュートインスタンスを作成することをお勧めします。 
...
### 2️⃣ リポジトリをクローンし、必要なパッケージをインストールします
1. お使いの端末に移動し、リポジトリのクローンを作成します。
